...
#######################################################################################################
if_find_boundary=1
if(if_find_boundary==1):
    #### Update
    sub_dir=new_shape_path
    newDf=Read_SHAPE_File(current_path,'\\',output_name)
    ...
    dfDummy=newDf[newDf['tambon_idn'].isin(['102901'])].copy().reset_index(drop=True)
    # dfDummy=newDf[newDf['SUBDISTRIC'].isin(['102902'])].copy().reset_index(drop=True)
    geometry=str(list(dfDummy['geometry'].values)[0])
    geometry=geometry.replace("POLYGON",'')
    geometry=geometry.replace("((",'')
    geometry=geometry.replace("))",'')
    # print(' geometry : ',geometry,' :: ',type(geometry))

    coorList=geometry.split(',')
    print(' List : ',coorList)
    latList=[];lngList=[]
    lat_lngList=[]
    for coor in coorList:
        lat_lngList.append(coor)
        latList.append(float(coor.strip().split(' ')[1]))
        lngList.append(float(coor.strip().split(' ')[0]))

    print(latList,' :: ',lngList)

    pointDf=pd.DataFrame(zip(lat_lngList,latList,lngList), columns=['id','Lat','Lng'])
    print(' point : ',pointDf)
    gdf_point=Generate_Geopandas(pointDf, 'Lat', 'Lng')
    gdf_point.drop(columns=['coords'],inplace=True)
    print(' ==> ',gdf_point)
    gdf_point.to_excel(current_path+'\\'+'102901_location_update1.xlsx',index=False)
    gdf_point.to_file(current_path+'\\'+'boundary_point.shp', encoding='utf-8') 

def Generate_new_boundary():
...
# gdf_main=Generate_new_boundary()
